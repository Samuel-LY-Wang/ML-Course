...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    ...
    return score(data_test, labels_test, th, th0)/data_test.shape[1], (th, th0)
...
def xval_learning_alg(learner, data, labels, k, T):
    ...
    for none in range(T):
        ...
        for i in range(k):
            ...
            eval, ths=eval_classifier(learner, data_train, labels_train, data_test, labels_test)
            score_sum += eval
            # print(ths[0])
            # print(ths[1])
            avg_th += ths[0].T[0]
            avg_th0 += ths[1][0]
        totalacc += score_sum
        # print(f"Training session {none} done")
    return totalacc/(k*T), (avg_th/(k*T), avg_th0/(k*T))
...
def get_classification_accuracy(data, labels):
    ...
    """
    return xval_learning_alg(lambda data, labels: perceptron(data, labels, {"T": 50}), data, labels, 10, 50)
...
def load_mnist_data(labels):
    ...
    for label in labels:
        images = load_mnist_single("code_and_data_for_hw3/mnist/mnist_train{}.png".format(label))
